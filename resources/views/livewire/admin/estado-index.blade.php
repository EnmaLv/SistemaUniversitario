    ...
    <div class="overflow-hidden rounded-2xl border shadow-sm" style="background-color: var(--bg-card); border-color: var(--border-color);">
        <div class="border-b px-5 py-4" style="background-color: var(--bg-card); border-color: var(--border-color);">
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    <h3 class="text-lg font-semibold" style="color: var(--text-main);">Estados Registrados</h3>
                </div>
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <div class="flex items-center gap-2 rounded-xl border px-3 py-2" style="background-color: var(--input-bg); border-color: var(--border-color);">
                        <input type="text" class="w-full border-0 bg-transparent text-sm outline-none placeholder:text-gray-400" style="color: var(--text-main);" placeholder="Escriba un Estado" wire:model.live.debounce.300ms="search" />
                ...
                <tbody class="divide-y" style="divide-color: var(--border-color);">
                    @forelse ($estados as $index => $datos)
                        <x-table-row :id="$datos->id">
                            ...
                            <td class="px-4 py-3" style="color: var(--text-main);">{{ $datos->nombre_estado }}</td>
                            <td class="px-4 py-3">
                                @if ($datos->status)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="acciones-wrap relative flex h-8 items-center justify-center">
                                    <div id="trigger-estado-{{ $datos->id }}" class="acciones-trigger flex h-8 w-8 items-center justify-center rounded-xl border text-gray-500 shadow-sm transition-all hover:bg-rose-50 hover:text-rose-600 dark:border-gray-600/50 dark:text-gray-400 dark:hover:bg-rose-950/50">
                                        <i class="fas fa-ellipsis-vertical text-xs"></i>
                                    </div>
                                    <div id="panel-estado-{{ $datos->id }}" class="acciones-panel">
                        ...
                        <tr>
                            <td colspan="4" class="px-4 py-10">
                                <div class="flex flex-col items-center justify-center gap-3" style="color: var(--text-muted);">
                                    ...
                                    <h4 class="text-center text-lg font-semibold">No hay Estados registrados</h4>
                                    <p class="text-center text-sm">Agrega un nuevo estado con el botón superior</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
