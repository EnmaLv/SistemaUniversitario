        ...
        (() => {
            ...
            Livewire.on('confirm-delete', data => {
                Swal.fire({
                    ...
                    cancelButtonText: 'Cancelar', confirmButtonColor: '#991b1b',
        ...
        <div class="p-2.5 border-b" style="border-color: var(--border-color);">
            <div class="flex flex-col lg:flex-row lg:items-center gap-4">
                <div class="relative w-full">
                    ...
                    <input type="text" wire:model.live.debounce.300ms="search" placeholder="Buscar estado..."
                ...
                <label class="flex items-center gap-3 px-3 py-2 rounded-xl border shrink-0 cursor-pointer"
                    style="border-color: var(--border-color);">
                    <span class="text-[11px] font-black uppercase tracking-wider text-gray-500 dark:text-gray-400">Activos</span>
                    <input wire:model.live="filtroEstado" type="checkbox" class="sr-only peer">
                    <span class="w-10 h-6 rounded-full bg-gray-300 dark:bg-gray-700 peer-checked:bg-red-700 relative after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-4"></span>
                </label>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50/50 dark:bg-black/20 border-b border-gray-100 dark:border-gray-800 text-[13px] font-black uppercase tracking-wider">
                        ...
                        <th class="px-6 py-4 text-center">Estado del registro</th>
                        <th class="px-6 py-4 text-center" style="width:120px;">Acciones</th>
                    </tr>
                </thead>
                <tbody class="text-xs font-medium">
                    @forelse ($estados as $datos)
                        <x-table-row :id="$datos->id" class="border-b" style="border-color: var(--border-color);">
                            <td class="px-6 py-4 text-center whitespace-nowrap"><span class="inline-flex items-center px-3 py-1 text-[12px] font-black rounded-lg text-gray-600 dark:text-gray-300 border border-gray-300 dark:border-gray-800">{{ ($estados->currentPage()-1)*$estados->perPage()+$loop->iteration }}</span></td>
                            <td class="px-6 py-4 text-center whitespace-nowrap font-bold" style="color:var(--text-main);">{{ $datos->nombre_estado }}</td>
                            <td class="px-6 py-4 text-center whitespace-nowrap">
                                @if($datos->status)
                                    <span class="inline-flex items-center gap-1 px-3 py-1 text-[10px] font-black rounded-lg bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-900"><i class="fas fa-check-circle"></i> Activo</span>
                                @else
                                    <span class="inline-flex items-center gap-1 px-3 py-1 text-[10px] font-black rounded-lg bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-900"><i class="fas fa-times-circle"></i> Inactivo</span>
                                @endif
                            </td>
                            <x-table-actions :id="$datos->id" baseUrl="admin/estado" :status="$datos->status" :show="false" :edit="false" :toggle="false">
                    ...
                    @empty
                        <tr><td colspan="4" class="px-6 py-12 text-center text-xs font-bold uppercase tracking-wider text-gray-400">No hay estados registrados</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <div class="flex justify-center border-t p-4" style="border-color:var(--border-color);">{{ $estados->onEachSide(1)->links('components.pagination-livewire') }}</div>
    </div>
</div>
