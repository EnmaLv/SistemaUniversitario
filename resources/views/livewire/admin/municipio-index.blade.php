<div x-data="{ abrirCrear: false, abrirEditar: false }">
    ...
    <script>
        (() => {
            const registerMunicipioListeners = () => {
                Livewire.on('swal', data => {
                    if (data.title === '¡Éxito!') {
                        document.getElementById('modalCrearMunicipio')?.classList.add('hidden');
                        document.getElementById('modalCrearMunicipio')?.classList.remove('flex');
                    }
                    Swal.fire({ icon:data.icon,title:data.title,text:data.text,confirmButtonColor:'#991b1b',timer:3000,timerProgressBar:true });
                });
                Livewire.on('confirm-delete', data => Swal.fire({ title:'¿Estás seguro?',text:'¿Desea inactivar el municipio?',icon:'warning',showCancelButton:true,confirmButtonText:'Sí, inactivar',cancelButtonText:'Cancelar',confirmButtonColor:'#991b1b',cancelButtonColor:'#4b5563' }).then(result => { if(result.isConfirmed) Livewire.dispatch('destroy-municipio',{id:data.id}); }));
                Livewire.on('confirm-activate', data => Swal.fire({ title:'¿Reactivar municipio?',text:'El municipio volverá a estar disponible en el sistema.',icon:'question',showCancelButton:true,confirmButtonText:'Sí, reactivar',cancelButtonText:'Cancelar',confirmButtonColor:'#991b1b',cancelButtonColor:'#4b5563' }).then(result => { if(result.isConfirmed) Livewire.dispatch('activate-municipio',{id:data.id}); }));
            };

            if (window.Livewire) {
                registerMunicipioListeners();
            } else {
                document.addEventListener('livewire:init', registerMunicipioListeners, { once: true });
            }
        })();
    </script>
    <div style="background-color:var(--bg-card);border-color:var(--border-color);" class="rounded-2xl border shadow-sm overflow-hidden">
        ...
        <div class="overflow-x-auto" id="printArea"><table class="w-full text-left border-collapse">
            <thead><tr class="bg-gray-50/50 dark:bg-black/20 border-b border-gray-100 dark:border-gray-800 text-[13px] font-black uppercase tracking-wider"><th class="px-6 py-4 text-center" style="width:80px;">#</th><th class="px-6 py-4 text-center">Municipio</th><th class="px-6 py-4 text-center">Estado</th><th class="px-6 py-4 text-center">Estado del registro</th><th class="px-6 py-4 text-center" style="width:120px;">Acciones</th></tr></thead>
            <tbody class="text-xs font-medium">
                @forelse($municipios as $datos)
                    <x-table-row :id="$datos->id" class="border-b" style="border-color: var(--border-color);">
                        <td class="px-6 py-4 text-center whitespace-nowrap"><span class="inline-flex items-center px-3 py-1 text-[12px] font-black rounded-lg text-gray-600 dark:text-gray-300 border border-gray-300 dark:border-gray-800">{{ ($municipios->currentPage()-1)*$municipios->perPage()+$loop->iteration }}</span></td>
                        <td class="px-6 py-4 text-center whitespace-nowrap font-bold" style="color:var(--text-main);">{{ $datos->nombre_municipio }}</td>
                        <td class="px-6 py-4 text-center whitespace-nowrap text-gray-500 dark:text-gray-400">{{ $datos->estado->nombre_estado ?? 'Sin estado' }}</td>
                        ...
                        <x-table-actions :id="$datos->id" baseUrl="admin/municipio" :status="$datos->status" :show="false" :edit="false" :toggle="false"><button wire:click="edit({{ $datos->id }})" @click="abrirEditar=true" class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-amber-500 hover:bg-amber-100 dark:hover:bg-amber-950/50" title="Editar"><i class="fas fa-edit text-xs"></i></button>@if($datos->status)<button wire:click="confirmDestroy({{ $datos->id }})" type="button" class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-rose-500 hover:bg-rose-100 dark:hover:bg-rose-950/50" title="Inactivar"><i class="fas fa-trash-alt text-xs"></i></button>@else<button wire:click="confirmActivate({{ $datos->id }})" type="button" class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-emerald-500 hover:bg-emerald-100 dark:hover:bg-emerald-950/50" title="Reactivar"><i class="fas fa-undo text-xs"></i></button>@endif</x-table-actions>
                    </x-table-row>
                @empty<tr><td colspan="5" class="px-6 py-12 text-center text-xs font-bold uppercase tracking-wider text-gray-400">No hay municipios registrados</td></tr>@endforelse
            </tbody></table></div>
        <div class="flex justify-center border-t p-4" style="border-color:var(--border-color);">{{ $municipios->onEachSide(1)->links('components.pagination-livewire') }}</div>
    </div>
</div>
