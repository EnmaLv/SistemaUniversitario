<x-app-layout>
    <div class="min-h-[calc(100vh-4rem)] pb-12 pt-6">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs font-medium text-gray-500 dark:text-gray-400 sm:text-sm">
                            Modifique los datos, horarios o el trazado de la ruta de transporte.
                        </p>
                    </div>
                </div>
            ...
            @if ($errors->any())
                <div class="alert alert-danger shadow-sm mb-5" style="border-radius: 8px;">
            ...
            @if ($errors->has('error'))
                <div class="alert alert-danger mb-5"><b>{{ $errors->first('error') }}</b></div>
            @endif
            ...
            <form action="{{ route('admin.transporte.maestros.bus_rutas.update', $busRuta->id) }}" method="POST"
                ...
                <div class="grid grid-cols-1 gap-5 lg:grid-cols-5">
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <div class="route-card mb-4 rounded-2xl border p-5 shadow-sm"
                            ...
                            <div class="route-card-heading mb-4">
                                ...
                                <div>
                                    ...
                                    <p>Modifica los datos generales y la configuración de la ruta.</p>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="flex items-center rounded-xl border border-slate-200 bg-slate-50 mt-1"
                                    ...
                                    <input type="text" name="nombre" id="inputNombre"
                                        ...
                                        placeholder="Ej: Zona Sur - Directo"
                                        value="{{ old('nombre', $busRuta->nombre) }}" maxlength="100" required
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <div class="flex items-center rounded-xl border border-slate-200 bg-slate-50 mt-1"
                                            ...
                                            <input type="number" name="distancia_km" id="inputDistancia" step="0.01"
                                                ...
                                                value="{{ old('distancia_km', $busRuta->distancia_km) }}" min="0.1"
                                                required readonly
                                                style="border-color:var(--border-color); background-color:var(--input-bg); color:var(--text-main);">
                                        </div>
                                    </div>
                        ...
                        {{-- Card 2: Planificación de horarios --}}
                        <div class="route-card mb-4 rounded-2xl border p-5 shadow-sm"
                            style="background-color:var(--bg-card);border-color:var(--border-color);">
                            <div class="route-card-heading mb-4">
                                <span class="route-section-icon"><i class="fas fa-clock"></i></span>
                                <div>
                                    <h4 class="text-sm font-extrabold" style="color:var(--text-main);">
                        ...
                        <div class="route-card mb-4 rounded-2xl border p-5 shadow-sm"
                            ...
                            <div class="route-card-heading mb-4">
                                ...
                                <div>
                                    <h4 class="text-sm font-extrabold" style="color:var(--text-main);">Descripción</h4>
                            ...
                            <div class="form-group mb-0">
                                <textarea name="descripcion" class="route-textarea" rows="3" maxlength="1000"
                                    ...
                                    style="border-color:var(--border-color); background-color:var(--input-bg); color:var(--text-main);">{{ old('descripcion', $busRuta->descripcion) }}</textarea>
                                <div class="mt-1 text-right text-xs text-gray-500">
                                    {{ strlen(old('descripcion', $busRuta->descripcion ?? '')) }}/1000</div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="route-card mb-4 rounded-2xl border p-5 shadow-sm"
                            ...
                            <div class="mb-3">
                                <div class="route-card-heading">
                                    ...
                                    <div>
                                        ...
                                        <p>Modifica el orden de las paradas o agrega nuevas al recorrido.</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 gap-4">
                                ...
                                <div>
                                    <label class="rd-label">
                                        <i class="fas fa-list-ol mr-1"></i> Secuencia de paradas
                                    </label>
                            ...
                            @error('paradas')
                                <div class="text-danger mt-2">
                                    <b>Debe añadir al menos 2 paradas al trazado en el mapa.</b>
                                </div>
                            @enderror
                            ...
                            <div class="mt-4 flex items-center justify-end gap-3 border-t pt-4"
                                ...
                                <button type="submit"
                                    ...
                                    <i class="fas fa-save text-xs"></i> Actualizar ruta
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            <script>
                ...
                function initMap() {
                    ...
                    paradasDisponibles.forEach(parada => {
                        ...
                        marker.on('click', () => {
                            // Toggle: si ya está en la secuencia, quitarla
                    ...
                    // Cargar paradas: primero old(), si no, las de la BD
                ...
                async function actualizarInterfazYPolilinea() {
                    ...
                    try {
                        ...
                        if (data.code === 'Ok' && data.routes.length > 0) {
                            ...
                            if (inputDistancia) inputDistancia.value = kilometrajeReal;
                        } else {
                            throw new Error('OSRM no encontró una ruta para los puntos seleccionados.');
                        }
                    } catch (error) {
                        console.warn('No fue posible consultar OSRM; se usa distancia aproximada.', error);
                ...
                function normalizarHora(hora) {
                    ...
                    const coincidencia = valor.match(/^(\d{1,2}):(\d{2})(?::\d{2})?\s*(am|pm)?$/i);
                    if (!coincidencia) return valor.slice(0, 5);

                    let horas = Number(coincidencia[1]);
                    const minutos = coincidencia[2];
                    const periodo = coincidencia[3]?.toLowerCase();
                ...
                function agregarFilaHorario(hora = '', tipo = 'entrada') {
                    ...
                    fila.innerHTML = `
                        ...
                        <button type="button"
                            class="mt-5 inline-flex h-9 w-9 items-center justify-center rounded-lg bg-red-600 text-white transition hover:bg-red-700"
                ...
                document.addEventListener("DOMContentLoaded", function() {
                    ...
                    // Cargar horarios: old() si hay, si no los de la BD
