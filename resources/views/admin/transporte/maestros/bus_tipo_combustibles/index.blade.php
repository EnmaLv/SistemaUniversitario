    ...
    <div class="pt-6 pb-12 min-h-[calc(100vh-4rem)]">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-3 flex flex-col gap-4 rounded-2xl border p-2.5 shadow-sm lg:flex-row lg:items-center" style="background-color:var(--bg-card);border-color:var(--border-color);">
                <form action="{{ route('admin.transporte.maestros.bus_tipo_combustibles.index') }}" method="GET" class="relative w-full">
                        ...
                        <input type="text" name="buscar" value="{{ request('buscar') }}" class="w-full rounded-xl border py-2.5 pl-10 pr-4 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-red-500" style="background-color:var(--input-bg);border-color:var(--border-color);color:var(--text-main);" placeholder="Buscar tipo de combustible...">
                </form>
                ...
                <tbody class="divide-y text-xs font-medium">
                    @forelse($tipos as $tipo)
                        <x-table-row :id="$tipo->id" data-id="{{ $tipo->id }}">
                            ...
                            <x-table-actions :id="$tipo->id" baseUrl="admin/transporte/maestros/bus_tipo_combustibles" :show="false" :toggle="false"
                                :on-edit="'openEditarTipoByData(' . json_encode($tipo) . ')'">
                                    @if ($tipo->estado)
    ...
    <div id="modalCrear" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/60 p-4">
        <div class="w-full max-w-lg rounded-2xl border border-slate-200 bg-white shadow-2xl">
            ...
            <form id="formCrear" action="{{ route('admin.transporte.maestros.bus_tipo_combustibles.store') }}" method="POST" class="space-y-4 p-5">
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700">DescripciÃ³n <span class="text-xs text-slate-400">(opcional)</span></label>
                    <div class="relative">
        ...
        <div class="w-full max-w-lg rounded-2xl border border-slate-200 bg-white shadow-2xl">
            ...
            <form id="formEditar" action="" method="POST" class="space-y-4 p-5">
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-slate-500"><i class="fas fa-align-left"></i></span>
                        <input type="text" id="editDescripcion" name="descripcion" class="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-10 pr-3 text-sm text-slate-700 outline-none transition focus:border-red-500 focus:ring-2 focus:ring-red-500/20" placeholder="DescripciÃ³n del tipo" maxlength="255">
                    </div>
                </div>
                <div class="flex justify-end gap-3 border-t border-slate-200 pt-4">
