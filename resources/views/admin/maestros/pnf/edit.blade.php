...
@section('content_header')
    <div class="mb-6"><h1 class="text-2xl font-extrabold tracking-tight sm:text-3xl" style="color: var(--text-main);">Editar Programa de Formacion</h1><p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Actualiza la informacion del programa.</p></div>
@stop

@section('content')
    @include('components.alert')
    <div class="rounded-2xl border p-6 shadow-sm" style="background-color: var(--bg-card); border-color: var(--border-color);">
        <div class="mb-6 flex items-center justify-between border-b pb-4" style="border-color: var(--border-color);"><div><h2 class="text-lg font-bold" style="color: var(--text-main);">Datos del programa</h2><p class="text-sm text-gray-500 dark:text-gray-400">Modifique los datos necesarios.</p></div><a href="{{ route('admin.maestros.pnf.index') }}" class="rounded-xl border px-4 py-2 text-sm font-bold" style="border-color: var(--border-color); color: var(--text-main);"><i class="fas fa-arrow-left mr-1"></i> Volver</a></div>
